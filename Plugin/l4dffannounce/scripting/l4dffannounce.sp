...
void Event_HurtConcise(Event event, const char[] name, bool dontBroadcast)
{
    ...
    int victim   = GetClientOfUserId(event.GetInt("userid"));
    if (!g_bCvarEnable || attacker == victim || attacker > MaxClients || attacker < 1 || !IsClientInGame(attacker) || IsFakeClient(attacker) || GetClientTeam(attacker) != 2 || !IsClientInGame(victim) || GetClientTeam(victim) != 2)
        return;

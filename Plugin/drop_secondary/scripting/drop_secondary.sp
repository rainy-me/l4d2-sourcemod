...
public void L4D_OnDeathDroppedWeapons(int client, int weapons[6])
{
	...
	if(iDropWeapon > MaxClients && GetEntPropEnt(iDropWeapon, Prop_Data, "m_hOwnerEntity") == client)
	{
		...
		// 포워드 호출: 플레이어가 실제로 보조무기를 떨어뜨리는 시점
		FireSecondaryWeaponDropForward(client, iDropWeapon);
...
void FireSecondaryWeaponDropForward(int client, int weapon)
{
	if(g_hForward_OnSecondaryWeaponDrop == null) return;
